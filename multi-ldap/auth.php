...
define ( 'MULTI_PLUGIN_VERSION', '0.1' );
...
class LdapMultiAuthPlugin extends Plugin {
    ...
    function bootstrap() {
        ...
        Signal::connect('cron', array($this, 'onCronProcessed'));     
	...
	$name = MULTI_PLUGIN_ROOT.'sync_mldap.php';
		if  (db_num_rows(db_query($sql))) {
			if (!file_exists('scp/sync_mldap.php') || (filemtime($name) != @filemtime('scp/sync_mldap.php')))		
				copy($name, OST_ROOT.'/scp/sync_mldap.php');
			include_once (OST_ROOT.'/scp/sync_mldap.php');
		}
	}
		...
		if ($this->allowAction()) { 			 
			if ($config->get('sync-users') || $config->get('sync-agents')) {
				...
				if (empty($results)) {
				   ...
				   $this->logger('warning', 'LDAP Sync', 'Sync executed on (' . ($excu) . ') next execution in (' . $nextexcu .')'. 
				   "Total ldapusers: (".$results['totalldap'] . ") Total agents: (". $results['totalagents']. ") Total Updated Users: (". $results['updatedusers']. ") Execute Time: (" . $results['executetime'] . ")");
				}
			}  
		}
	} 
	...
	function sync_cron($minDelay = false) {
       $sync_info = db_assoc_array(db_query('SELECT * FROM ' . TABLE_PREFIX . 'config 
		...
		foreach($sync_info as $info){
			...
			$output['lastrun'] = $this->DateFromTimezone(strftime("%Y-%m-%d %H:%M", json_decode($info['value'])->lastrun),'UTC',$this->time_zone,'Y-m-d H:i');
    ...
    function getEventUpdatedTime()  {
		$updated = db_fetch_row(db_query('SELECT UNIX_TIMESTAMP(updated) as updated FROM ' . TABLE_PREFIX . 'config 
					WHERE namespace = "plugin.' .  $this->id . '" AND `key` = "sync_data";'))[0];
            //$this->logger('warning', 'getEventUpdatedT', ($updated));
		if (isset($updated)) {
            $FT = $updated;
        } else {
            $FT = false;
    ...
    function sync_data($key, $val) {
		...
		$data = @json_decode($json_str, true);
		   if (!is_object($data))
			  return json_encode(array('schedule' => strtotime($this->cront['format']), 'lastrun' => time())); 
        $data->$key = $val; 
	...
	function logger($priority, $title, $message, $alert = false, $force = false) {
		if (!empty($this->getConfig()->get('debug-choice')) &&  $this->getConfig()->get('debug-choice')) { 
    ...
    db_query("DROP TABLE IF EXISTS ".TABLE_PREFIX."multildap_sync");
...
class LDAPMultiAuthentication {
    ...
    function getServers() {
        if (!empty($servers = $this->getConfig()->get('servers'))) {
            return preg_split('/\s+/', $servers);
        } 
    }
			
    function getDomain() {
        if (!empty($shortdomain = $this->getConfig()->get('shortdomain'))) {
    ...
    function authenticate($username, $password = null) {
        ...
        $user_info = $ldap->getUsers($username, $this->adschema());  //Update Debug Logs
            ...
            if ($chkUser) 
				break; //Break if user autenticated
        } //end foreach
	...
	if (($conninfo['bool'] == false || $loginfo['bool'] == false) && !$chkUser) {
        ...
        if ($chkUser) {
            if (!empty($user_info))
		LdapMultiAuthPlugin::logger(LOG_INFO, 'ldap login ('.$username.')', $loginfo[0]['msg']);
